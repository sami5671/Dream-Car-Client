...
import { updateUserAddedCarInfo } from "../../../api/Cars";

const UpdateUserAddedCar = () => {
  ...
  const handleUpdateCar = async (e) => {
    ...
    try {
      const data = await updateUserAddedCarInfo(_id, carData);
      console.log(data);
      if (data.modifiedCount > 0) {
        toast.success(`${CarModel} Updated successfully`);
        setLoading(false);
        navigate("/dashboard/user-garage");
      }
    } catch (error) {
...
export default UpdateUserAddedCar;
